...
#define IMAGE_RGB_SIZE (160 * 120 * 2) /* QQVGA RGB565 */
...
static int camera_prepare(int fd, enum v4l2_buf_type type, uint32_t buf_mode,
                          uint32_t pixformat, uint16_t hsize, uint16_t vsize,
                          struct v_buffer **vbuf, uint8_t buffernum,
                          int buffersize);
static void free_buffer(struct v_buffer *buffers, uint8_t bufnum);
static int parse_arguments(int argc, char *argv[], int *capture_num,
...
int main(int argc, FAR char *argv[]) {
  ...
  if (capture_num != 0) {
    ...
    ret = camera_prepare(v_fd, V4L2_BUF_TYPE_STILL_CAPTURE, V4L2_BUF_MODE_FIFO,
                         V4L2_PIX_FMT_JPEG, w, h, &buffers_still, STILL_BUFNUM,
                         IMAGE_JPG_SIZE);
    if (ret != OK) {
      goto exit_this_app;
    }
  ...
  ret = camera_prepare(v_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, V4L2_BUF_MODE_RING,
                       V4L2_PIX_FMT_RGB565, VIDEO_HSIZE_QVGA, VIDEO_VSIZE_QVGA,
  ...
  while (1) {
...
#ifdef CONFIG_EXAMPLES_CAMERA_OUTPUT_LCD
      nximage_draw((void *)v4l2_buf.m.userptr, VIDEO_HSIZE_QVGA,
                   VIDEO_VSIZE_QVGA);
#endif
    ...
    case APP_STATE_UNDER_CAPTURE:
      ...
      while (capture_num) {
        ...
        futil_writeimage(
            (uint8_t *)v4l2_buf.m.userptr, (size_t)v4l2_buf.bytesused,
